...
public class XMLReader {
        ...
        try {
            ...
            for (int i = 0; i < groupNodes.getLength(); i++) {
                ...
                if (node.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    try {
                        collection.add(parseGroup(element));
                    } catch (Exception e) {
                        String idFailedGroup = getDirectChildValue(element, "id");
                        if (idFailedGroup != null && !idFailedGroup.trim().isEmpty()){
                            System.out.println("Группа с id = " + idFailedGroup + " пропущена, ошибка " + e.getMessage());
                        }
                        else System.out.println("Ошибка, группа пропущена " + e.getMessage());
                    }
                }
            }
        } catch (Exception e) {
     ...
     * @return полностью сформированный и проверенный на валидацию объект StudyGroup
